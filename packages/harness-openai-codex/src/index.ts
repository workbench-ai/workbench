  ...
  implements HarnessAdapter<CodexSessionState>
{
  ...
  static requireApiKey(
    ...
    return CodexHarnessAdapter.resolveApiKeyAuth(plan, repoRoot, flowHome)
      .apiKey;
  }
  ...
  static resolveApiKeyAuth(
    ...
    if (!apiKey) {
      ...
      throw new Error(
        `${auth.ref} must be set in ${location} before running Flow Codex sessions.`,
      );
    }
    return {
  ...
  static async ensureAuthReady(
    ...
    if (auth.strategy === "secret_ref") {
      CodexHarnessAdapter.resolveApiKeyAuth(plan, repoRoot, flowHome);
      return;
    }
