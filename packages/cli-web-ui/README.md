...
This root owns the reusable styling, shadcn-based primitives, shell helpers, and read-only file preview surfaces that shared consumers import. That shared preview layer now includes the Codex-style `.xlsx` spreadsheet viewer under `components/shared/spreadsheet-viewer.tsx` plus the parser/runtime helpers under `lib/spreadsheet-viewer*.ts`, with a unified consumer entrypoint at `@workbench-ai/cli-web-ui/spreadsheet-viewer`. Product-specific behavior, routes, branding, and runtime semantics stay in the owning product roots.
