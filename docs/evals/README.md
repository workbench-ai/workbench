...
`workbench new` creates the eval definition and an empty `.workbench/cases/` directory. Add at least one case before running `workbench eval`; a real eval with no cases fails with `no_eval_cases` and prints `workbench case draft CASE_ID`, while `eval --dry-run` still previews `cases=0` plus any auth readiness without writing state. `workbench case draft CASE_ID` creates a draft case plus an executable `tests/test.sh` harness, then prints an editor `next` command for both generated files. The harness intentionally fails with score `0` until the placeholder prompt, rubric, and assertions are replaced.
