...
Use the first run to verify that verifier files, durable artifacts, traces, and candidate file changes are all understandable before increasing budget or samples. The web view is read-only inspection; execution, retry, push, and pull remain CLI actions.
