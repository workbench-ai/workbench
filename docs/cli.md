...
`workbench open` serves the shared read-only inspection model as a foreground server. `--port 0` is valid and prints the OS-assigned bound port plus the `Press Ctrl-C to stop` hint. Use `log` and `show REF` for summary inspection, and `show REF:PATH` for content reads. The web `Compare` surface is the browser counterpart to `workbench compare`; write actions remain in the CLI.
