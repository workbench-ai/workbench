...
Human `results` output reports average per-sample latency when a run has more than one sample, so a five-sample run is not displayed as a single inflated aggregate latency. Stored run evidence keeps the original aggregate timing.
