...
`local` and `command` agents run Docker-style case tests directly, and local provider-backed eval/improve still uses the Docker sandbox around the adapter runtime. `codex` and `claude` agents run the provider as the skill executor and score the same cases through the configured score adapter. Connect provider auth before using provider-backed agents locally or in Cloud.
