...
Use `workbench run` to inspect model output before finalizing rubric criteria. Prompt, public case input, source, agent, and sample changes make execution stale; rubric, grade adapter, grader config, and private grader-file changes make grading stale only. Repeated `workbench run` reuses current execute evidence unless `--rerun` is passed. `workbench grade` reuses eligible execute jobs and never reruns Codex; `run --rerun` forces fresh execution and `grade --rerun` forces only grading. Every grade job starts from a clean workspace with the subject output, workspace, traces, and result mounted read-only, so regrading does not inherit prior grade-job state.
