...
- Include a workbook parser such as `openpyxl` or an equivalent when the subject runner or scorer needs to inspect sheets, cells, formulas, tables, or workbook structure.
...
- Cached formula values may be out of date or missing unless an Office engine recalculates or refreshes the workbook.
