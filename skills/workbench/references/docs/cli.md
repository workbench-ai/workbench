...
In JSON mode, `new` includes `createdPaths`, `defaultAgentSelection`, `setupCommands`, and `next`; while no workflow case exists, `next` is `workbench case draft CASE_ID`, which creates a draft `.workbench/cases/CASE_ID/case.yaml` plus an executable `tests/test.sh` harness. When `workbench new DIR` is run from outside the created project, the printed next command is scoped with `cd DIR && ...` so it is safe to run from the same shell.
