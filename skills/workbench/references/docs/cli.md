...
`skills` is the inventory command. With no flags, it answers what the current coding agent can access in the current folder. `skills --global` answers what that agent can access globally. `skills --for all` shows Codex and Claude access in the folder, and `skills --for all --global` shows Codex and Claude global access. Inventory is read-only, performs no network access, and reports `current`, `modified`, `missing`, `unmanaged`, or `duplicate-name` status when visible.
